...
def get_omm(sat: str):  
    ...
    if not load.exists(DATA_CSV) or load.days_old(DATA_CSV) >= DAYS_UPDATE:
        load.download(url, filename=DATA_CSV)

    fields = search_csv(sat, DATA_CSV)
...
def read_data(raw_text, units_library):
    """
    Парсит ввод вида: 
    UNIT_ID: arg1, arg2
    ...
    library_map = {u.unit_id: u for u in units_library}
