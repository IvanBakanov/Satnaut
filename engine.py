...
class Satellite:
    ...
    def get_orbital_period(self):
        rad_per_min = self.satrec.no_kozai
        
        period = (2 * np.pi) / rad_per_min
...
# Функция для отрисовки подспутниковой трассы. period в минутах
def trace_simulation(sat, now, period):
    lats = []
    lons = []
    ...
    for i in range(int(period) + 1):
        ...
        state = sat.get_state_vector(current_time)
        
        lat, lon = sat.get_subsatellite_point(state, current_time)
